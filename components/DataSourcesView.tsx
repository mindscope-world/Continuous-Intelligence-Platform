    ...
    <main className="flex-1 p-8 overflow-y-auto animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex gap-3">
             <button className="px-4 py-2 bg-[#18181b] border border-zinc-800 text-zinc-300 hover:text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
      ...
      <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 mb-8 flex items-start gap-3">
         ...
         <div>
            <h4 className="text-sm font-bold text-green-400">Enterprise Security Active</h4>
      ...
      {/* Filter Bar */}
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6 bg-[#18181b] p-2 rounded-xl border border-zinc-800">
         <div className="relative w-full sm:w-96">
            ...
            <input 
              ...
              className="w-full bg-[#09090b] text-zinc-300 text-sm rounded-lg pl-9 pr-3 py-2 border border-zinc-800 focus:outline-none focus:border-zinc-700 transition-colors"
            />
         </div>
         <div className="flex gap-2">
            {['All', 'Connected', 'Disconnected'].map((filter) => (
               <button key={filter} className="px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors">
                  {filter}
               </button>
            ))}
         </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredSources.map((source) => (
           <div key={source.id} className="bg-[#18181b] border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-all group flex flex-col h-full">
              
              <div className="flex justify-between items-start mb-4">
                 <div className={`w-12 h-12 rounded-xl ${source.iconColor} bg-opacity-10 flex items-center justify-center text-white font-bold text-xl shadow-inner`}>
                    {/* Placeholder for Logos using text initials or generic icons */}
              ...
              <div className="mb-4 flex-1">
                 <h3 className="text-white font-bold text-lg mb-1">{source.name}</h3>
                 <p className="text-xs text-zinc-500 mb-2">{source.category}</p>
                 <p className="text-xs text-zinc-400 leading-relaxed min-h-[40px]">{source.description}</p>
              </div>
        ...
        {/* Add New Placeholder */}
        <button className="border border-dashed border-zinc-800 rounded-xl p-5 flex flex-col items-center justify-center gap-3 hover:bg-zinc-900/50 hover:border-zinc-700 transition-all group min-h-[240px]">
           <div className="w-12 h-12 rounded-full bg-zinc-900 flex items-center justify-center group-hover:scale-110 transition-transform">
              <Plus size={24} className="text-zinc-600 group-hover:text-zinc-400" />
           </div>
           <div className="text-center">
              <h3 className="text-zinc-400 font-medium text-sm group-hover:text-white">Connect New Source</h3>
              <p className="text-xs text-zinc-600 mt-1">Browse 50+ integrations</p>
           </div>
        </button>
      </div>
    </main>
