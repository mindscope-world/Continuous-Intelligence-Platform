...
const AnalyzeView: React.FC = () => {
  return (
    <main className="flex-1 p-8 overflow-y-auto animate-in fade-in duration-300">
      ...
      <div className="bg-gradient-to-r from-blue-900/20 to-indigo-900/20 border border-blue-500/20 rounded-2xl p-6 mb-8 relative overflow-hidden">
        ...
        <div className="flex items-start gap-4 relative z-10">
          ...
          <div>
            <h3 className="text-lg font-bold text-white mb-2">Executive Summary (Q3)</h3>
            <p className="text-zinc-300 text-sm leading-relaxed max-w-4xl">
              Revenue is tracking <span className="text-green-400 font-bold">12% above forecast</span>, primarily driven by strong Enterprise expansion. 
              However, <span className="text-red-400 font-bold">Mid-Market churn</span> has increased slightly due to onboarding delays. 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-[#18181b] border border-zinc-800 rounded-2xl p-6">
           <div className="flex justify-between items-center mb-6">
              <h3 className="font-semibold text-white">Revenue Drivers (Impact %)</h3>
        ...
        <div className="bg-[#18181b] border border-zinc-800 rounded-2xl p-6">
           <div className="flex justify-between items-center mb-6">
              <div>
                 <h3 className="font-semibold text-white">Spend vs. Lead Volume</h3>
                 <p className="text-xs text-zinc-500">Correlation Analysis</p>
              </div>
              <div className="text-xs bg-zinc-800 px-2 py-1 rounded text-zinc-300">R² = 0.84</div>
           </div>
           <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
      ...
      {/* Insights Grid */}
      <h3 className="text-lg font-bold text-white mb-4">Key Anomalies & Insights</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {INSIGHTS.map((insight) => (
           <div key={insight.id} className="bg-[#18181b] border border-zinc-800 rounded-xl p-5 hover:border-zinc-700 transition-colors">
              <div className="flex justify-between items-start mb-3">
                 <span className={`p-2 rounded-lg ${
                    insight.type === 'positive' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'
                 }`}>
                    {insight.type === 'positive' ? <TrendingUp size={18} /> : <ArrowDownRight size={18} />}
                 </span>
                 <div className="text-right">
                    <div className={`text-lg font-bold ${
                        insight.type === 'positive' ? 'text-green-500' : 'text-red-500'
                    }`}>{insight.impact}</div>
